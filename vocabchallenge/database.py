...
def get_top(x):
    cur = g.database.cursor()
    cur.execute('SELECT users.username, MAX(scores.score) AS highscore, scores.language FROM scores, users WHERE users.id=scores.userid GROUP BY users.username, scores.language ORDER BY highscore DESC LIMIT %s', [x])
