...
def get_top(x):
    cur = g.database.cursor()
    cur.execute('SELECT users.username, MAX(scores.score) AS highscore, scores.language FROM scores, users WHERE users.id=scores.userid GROUP BY users.username ORDER BY highscore DESC, scores.language DESC  LIMIT %s', [x])
    topx = [dict(username=row[0], score=str(row[1])+"\t"+row[2].capitalize()) for row in cur.fetchall()]
    cur.close()
    return topx
